...
export default function NewPostScreen() {
  ...
  const handlePost = async () => {
    if (!selectedImage && !content.trim()) {
      Alert.alert("กรุณากรอกข้อมูล", "กรุณาใส่รูปภาพหรือข้อความเพื่อแชร์เรื่องราวของคุณ");
      return;
    }

    setLoading(true);
    try {
      // ตรวจสอบว่าเป็นประกาศ Official หรือไม่ (Admin โพสต์ในคณะตัวเอง หรือ UniAdmin โพสต์ทั่วไป)
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: bgColor }}>
      ...
      >
        ...
        <TouchableOpacity onPress={handlePost} disabled={loading || (!selectedImage && !content)}>
           <Text className={`font-black ${(loading || (!selectedImage && !content)) ? 'opacity-20' : ''}`} style={{ color: theme.colors.primary }}>แชร์</Text>
        </TouchableOpacity>
      </View>
