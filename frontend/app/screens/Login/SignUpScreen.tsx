...
export default function SignUpScreen() {
  ...
  const handleVerifyEmail = () => {
    ...
    if (!dpuEmailRegex.test(email.toLowerCase())) {
      Alert.alert(
        "อีเมลไม่ถูกต้อง",
        "กรุณาใช้อีเมลในรูปแบบ 12345678@dpu.ac.th เท่านั้น",
      );
      return;
    }
  ...
  const handleFinishSignUp = async () => {
    if (!fullName.trim()) {
      Alert.alert("ข้อผิดพลาด", "กรุณากรอกชื่อของท่าน");
      return;
    }

    if (!username.trim()) {
      Alert.alert("ข้อผิดพลาด", "กรุณากรอกชื่อผู้ใช้ของท่าน");
      return;
    }
    ...
    if (password !== confirmPassword) {
      Alert.alert(
        "รหัสผ่านไม่ตรงกัน",
        "กรุณาตรวจสอบและกรอกรหัสผ่านให้ตรงกันทั้งสองช่อง",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert("สมัครสำเร็จ", "บัญชีของคุณพร้อมใช้งานแล้ว", [
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View
        className="absolute top-[-50] left-[-50] w-[300] h-[300] rounded-full opacity-20"
        style={{ backgroundColor: pastelBlue }}
      />
      <View
        className="absolute bottom-[-100] right-[-50] w-[400] h-[400] rounded-full opacity-30"
              ...
              {step === 1 && (
                <View>
                  <View className="mb-12">
                    <Text
                      className="text-4xl font-black leading-[50px] tracking-tight"
                      style={{ color: "#1A202C" }}
                    >
                      เริ่มต้นใช้งาน{"\n"}
              ...
              {step === 2 && (
                <View>
                  <View className="mb-8 items-center">
                    ...
                    <Text className="text-slate-400 font-bold mt-2 text-center">
                      ใช้อีเมลมหาวิทยาลัยเพื่อยืนยันตัวตน
                    </Text>
                  </View>

                  <View className="space-y-4">
                    <View className="flex-row items-center rounded-3xl px-6 py-5 bg-slate-100 border-2 border-slate-200 focus:border-[#7B61FF]">
                      <Ionicons name="mail-outline" size={20} color="#7B61FF" className="mr-3" />
                      <TextInput
                        placeholder="12345678@dpu.ac.th"
              ...
              {step === 3 && (
                <View>
                  <View className="mb-10">
                    ...
                    >
                      สร้าง{"\n"}
                      <Text style={{ color: "#7B61FF" }}>บัญชีใหม่</Text>
                    </Text>
                    <Text className="text-lg mt-3 font-bold opacity-30 tracking-wide">
                      ตั้งค่าชื่อผู้ใช้และรหัสผ่านเพื่อเข้าใช้งานระบบ
                    </Text>
                  </View>
